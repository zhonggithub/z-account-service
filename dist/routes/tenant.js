...
exports.default = {
  create(ctx) {
    return _asyncToGenerator(function* () {
      ...
      // 生成 唯一的tanent key
      body.key = `zt${ (0, _moment2.default)().format('YYYYMMDDHHmmss') }${ Math.random().toString().slice(-6) }`;
      body.secret = _common.util.md5Encode(_common.util.aesEncode(body.secret, _common.config.aesKey));
  ...
  logicDelete(ctx) {
    return _asyncToGenerator(function* () {
      const id = ctx.params.id;
      const ret = yield prp.resourceProxy.logicDeleteById(id);
      if (!ret) {
        const err = _common.common.error404();
        err.description = `You can't to delete resource because the resource of url(${ ctx.url }) is not exist.`;
        ctx.throw(err, 404);
